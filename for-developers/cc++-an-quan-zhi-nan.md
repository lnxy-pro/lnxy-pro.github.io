...
* [1 通用安全指南](broken-reference)
  * [I. C/C++使用错误](broken-reference)
    * [1.1 不得直接使用无长度限制的字符拷贝函数](broken-reference)
    * [1.2 创建进程类的函数的安全规范](broken-reference)
    * [1.3 尽量减少使用 \_alloca 和可变长度数组](broken-reference)
    * [1.4 printf系列参数必须对应](broken-reference)
    * [1.5 防止泄露指针（包括%p）的值](broken-reference)
    * [1.6 不应当把用户可修改的字符串作为printf系列函数的“format”参数](broken-reference)
    * [1.7 对数组delete时需要使用delete\[\]](broken-reference)
    * [1.8 注意隐式符号转换](broken-reference)
    * [1.9 注意八进制问题](broken-reference)
  * [II. 不推荐的编程习惯](broken-reference)
    * [2.1 switch中应有default](broken-reference)
    * [2.2 不应当在Debug或错误信息中提供过多内容](broken-reference)
    * [2.3 不应该在客户端代码中硬编码对称加密秘钥](broken-reference)
    * [2.4 返回栈上变量的地址](broken-reference)
    * [2.5 有逻辑联系的数组必须仔细检查](broken-reference)
    * [2.6 避免函数的声明和实现不同](broken-reference)
    * [2.7 检查复制粘贴的重复代码](broken-reference)
    * [2.8 左右一致的重复判断/永远为真或假的判断](broken-reference)
    * [2.9 函数每个分支都应有返回值](broken-reference)
    * [2.10 不得使用栈上未初始化的变量](broken-reference)
    * [2.11 不得直接使用刚分配的未初始化的内存（如realloc）](broken-reference)
    * [2.12 校验内存相关函数的返回值](broken-reference)
    * [2.13 不要在if里面赋值](broken-reference)
    * [2.14 确认if里面的按位操作](broken-reference)
  * [III. 多线程](broken-reference)
    * [3.1 变量应确保线程安全性](broken-reference)
    * [3.2 注意signal handler导致的条件竞争](broken-reference)
    * [3.3 注意Time-of-check Time-of-use条件竞争](broken-reference)
  * [IV. 加密解密](broken-reference)
    * [4.1 不得明文存储用户密码等敏感数据](broken-reference)
    * [4.2 内存中的用户密码等敏感数据应该安全抹除](broken-reference)
    * [4.3 rand() 类函数应正确初始化](broken-reference)
    * [4.4 在需要高强度安全加密时不应使用弱PRNG函数](broken-reference)
    * [4.5 自己实现的rand范围不应过小](broken-reference)
  * [V. 文件操作](broken-reference)
    * [5.1 避免路径穿越问题](broken-reference)
    * [5.2 避免相对路径导致的安全问题](broken-reference)
    * [5.3 文件权限控制](broken-reference)
  * [Ⅵ. 内存操作](broken-reference)
    * [6.1 防止各种越界写](broken-reference)
    * [6.2 防止任意地址写](broken-reference)
  * [Ⅶ. 数字操作](broken-reference)
    * [7.1 防止整数溢出](broken-reference)
    * [7.2 防止Off-By-One](broken-reference)
    * [7.3 避免大小端错误](broken-reference)
    * [7.4 检查除以零异常](broken-reference)
    * [7.5 防止数字类型的错误强转](broken-reference)
    * [7.6 比较数据大小时加上最小/最大值的校验](broken-reference)
  * [Ⅷ. 指针操作](broken-reference)
    * [8.1 检查在pointer上使用sizeof](broken-reference)
    * [8.2 检查直接将数组和0比较的代码](broken-reference)
    * [8.3 不应当向指针赋予写死的地址](broken-reference)
    * [8.4 检查空指针](broken-reference)
    * [8.5 释放完后置空指针](broken-reference)
    * [8.6 防止错误的类型转换](broken-reference)
    * [8.7 智能指针使用安全](broken-reference)
...
不应直接使用legacy的字符串拷贝、输入函数，如strcpy、strcat、sprintf、wcscpy、mbscpy等，这些函数的特征是：可以输出一长串字符串，而不限制长度。如果环境允许，应当使用其\_s安全版本替代，或者使用n版本函数（如：snprintf，vsnprintf）。

若使用形如sscanf之类的函数时，在处理字符串输入时应当通过%10s这样的方式来严格限制字符串长度，同时确保字符串末尾有\0。如果环境允许，应当使用\_s安全版本。
...
在 C++ 代码中，使用 `string`、`vector`、智能指针（比如[std::unique\_ptr\<T\[\]>](https://zh.cppreference.com/w/cpp/memory/unique\_ptr)）等可以消除绝大多数 `delete[]` 的使用场景，并且代码更清晰。
...
对于 C 代码，`C11` 后推荐使用 [atomic](https://en.cppreference.com/w/c/atomic) 标准库。 对于 C++代码，`C++11` 后，推荐使用 [`std::atomic`](https://zh.cppreference.com/w/cpp/atomic/atomic)。
...
https://libsodium.gitbook.io/doc/password\_hashing/default\_phf#example-2-password-storage
...
1. OpenSSL 中的 `RAND_bytes()` 函数, https://www.openssl.org/docs/man1.1.1/man3/RAND\_bytes.html
...
例如 C 标准库中的 `rand()` 的实现只是简单的[线性同余算法](https://sourceware.org/git/?p=glibc.git;a=blob;f=stdlib/random\_r.c;hb=glibc-2.28#l353)，生成的伪随机数具有较强的可预测性。

当需要实现高强度加密，例如涉及通信安全时，不应当使用 `rand()` 作为随机数发生器。

实际应用中，[ C++11 标准提供的`random_device`保证加密的安全性和随机性](https://docs.microsoft.com/en-us/cpp/standard-library/random-device-class?redirectedfrom=MSDN\&view=vs-2019#remarks) 但是 [C++ 标准并不保证这一点](https://stackoverflow.com/questions/44867500/is-stdrandom-device-cryptographic-secure)。跨平台的代码可以考虑用 [OpenSSL](https://wiki.openssl.org/index.php/Random\_Numbers) 等保证密码学安全的库里的随机数发生器。
